...
int esp_emac_init(void)
{
  ...
  /* ESP-IDF suppresses its automatic SECONDARY esp_timer initializer when
   * built for NuttX.  esp_eth still uses esp_timer for link polling, so
   * initialize it explicitly after the NuttX scheduler is running.
   */
