...
- 最后更新：2026-09-06
- 当前阶段：M5 RJ45 核心查询与导出已通过实板验收并归档；返回 M3 完成三类样本标定，M5 工程指标仍待补齐
- 比赛截止：2026-09-20
- 功能冻结目标：2026-09-18
...
| Ethernet | M5 核心归档通过 | `eth0=10.0.0.2/24` 自动配置；状态、完整 JSONL、FAIL BMP、路径保护、网络负载下本地检测和断线恢复实板通过 | 补齐 heap/任务栈等工程指标；图形化网页为可选增强 |
...
- [x] FAIL 时保存灰度 BMP 和差异框信息（文件、大小和 JSON 坐标已验证；Windows
      视觉打开检查并入 M6）。
- [x] 左侧显示最近检测历史（编号恢复及续接通过；最近 3 条精确顺序并入 M6）。
...
通过，详见 `board/contest_board/docs/m4-storage-archive.md`；PC 打开 FAIL BMP、
最近 3 条历史顺序及批量压力测试保留到 M6。
...
M5 RJ45 核心查询与导出已于 2026-09-06 完成实板验收并归档。当前返回 M3：

1. 固定摄像头、工件位置和照明，确定正确件、漏装件、错位件三类样本。
2. 每类检测不少于 20 次，记录得分、差异比例、PASS/FAIL、帧率和检测耗时。
3. 统计正确率、误判和漏判，据结果固定阈值及 ROI；不以单次样本代替统计。
4. 补齐 M5 heap、关键任务栈余量和固件大小记录；图形化网页仅作为可选演示增强。
5. M3 退出条件满足后进入 M6 的 100 次或 1 小时稳定性与三次冷启动验收。
