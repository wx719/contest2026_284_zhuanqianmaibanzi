...
- 最后更新：2026-09-06
- 当前阶段：离线检测与存储基线已归档；先完成 M3 三类样本标定，再实现比赛描述承诺的 RJ45 记录查询与导出
- 比赛截止：2026-09-20
- 功能冻结目标：2026-09-18
...
当前离线检测、显示、触摸和 MicroSD 留档基线已于 2026-09-06 归档，
详见 `board/contest_board/docs/pre-network-export-baseline-archive.md`。下一步返回
M3，以当前 18% 面积阈值和 12% 块异常阈值为起点完成实板标定：

1. 固定相机、工装、照明和标准件位置，重新录入三帧平均模板。
2. 正确件、漏装件、错位件各检测不少于 20 次，记录真实类别、判定、得分、差异
   比例、框位置和耗时。
3. 统计正确率、误判和漏判；优先调整 ROI、面积阈值和块异常阈值，不改变已经冻结
   的采集、预览和存储架构。
4. 同时记录预览帧率，要求不低于 8 fps，单次检测不超过 1 s。
5. M3 退出后进入 M5；Windows BMP 视觉检查、最近 3 条历史顺序、三次冷启动和
   连续压力测试统一在 M6 完成。
