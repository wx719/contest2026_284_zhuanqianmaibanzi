...
M4 和 M5 核心链路均已归档。当前返回 M3，在固定工装和照明下完成正确、漏装、
错位三类各不少于 20 次的阈值标定和统计；随后补齐 M5 工程指标并进入 M6 稳定性
验收。图形化导出网页属于可选演示增强，现有 JSON/JSONL/BMP API 已满足核心查询
与导出功能。
